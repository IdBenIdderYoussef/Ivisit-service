...
import ensa.mobile.ivisitservice.beta.model.*;
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        Account account = Account.builder().email("admin@mail.com").username("admin").password(
                ...
                .build();
        //aniss
        Address address= Address.builder().city("Marrakech").country("Maroc").build();
        Address address2= Address.builder().city("Safi").country("Maroc").build();
        //aniss-fin
        ...
        Post post = Post.builder().description("So here's what happened... Apparently we were so engaged in tonight " +
                ...
                " please feel free to follow with any").title("Marrakech City").address(address).build();
        ...
        Post post1 = Post.builder().description("Tu es doué en programmation ? Et tu es à la recherche d'une nouvelle " +
                ...
                " auprès de l'une de nos SSII partenaires.").title("Oued Zem : Madina chohada2").address(address2).build();
